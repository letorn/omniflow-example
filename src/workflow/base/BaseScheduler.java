...
import com.omniselling.wf.data.scheduler.BaseSchedulerData;
import com.omniselling.wf.data.scheduler.IntruptEvent;
import com.omniselling.wf.data.scheduler.UpdateSchedulerRequest;
import com.omniselling.wf.enumeration.scheduler.SchedulerStatus;
import com.omniselling.wf.enumeration.scheduler.TriggerType;
...
public abstract class BaseScheduler<D extends BaseSchedulerData> extends BasePersistentWorkflow<D>
{
    ...
    protected boolean handleResponse(D data, Response<IntruptEvent> interruptRes)
    {
        ...
        try
        {
            ...
            else
            {
                ...
                String msg;
                logger.info(
                        "Received INTRUPT, type=" + event.getResultEnum() + ", operatorId=" + event.getProcessedBy());
                auditLog(LogLevel.INFO, context,
                        "Received INTRUPT, type=" + event.getResultEnum() + ", operatorId=" + event.getProcessedBy());
                ...
                case RUNNOW:
                    auditLog(LogLevel.INFO, context,
                            "Manual performTask() invoked by " + event.getProcessedBy() + " note=" + event.getNote());
                    performTask(data);
                    break;
                default:
        ...
        catch (Exception e)
        {
            logger.error(e);
            e.printStackTrace();
            auditLog(LogLevel.ERROR, context,
    ...
    protected long evaluateNextTimeoutInterval(final D data, boolean noCatchUp)
    {
        ...
        {
            Date nextRunTime = noCatchUp ? getNextRunTime(data.getCronExp(), new Date())
                    : getNextRunTime(data.getCronExp(), lastRunTime);
