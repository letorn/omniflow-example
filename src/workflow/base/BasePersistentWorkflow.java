...
public abstract class BasePersistentWorkflow<E extends BaseWorkflowData> extends PersistentWorkflow<E>
{
     ...
     * <li>Put each step in a private function, each step must have its business meaning</li>
     * <li>Workflow data must extend {@link BaseWorkflowData}</li>
     * <li>Make external calls through adapters, adapters ref to external services and models</li>
     * <li>Use {@link #waitForResponse(long, String) waitForResponse(...)} to wait for Async responses from engine, and
     * {@link #notifyEngineSync(String, Object, String) notifyEngineSync(...)} to send response to engine to resume
     * workflow in wait status</li>
     * <li>In more complicated scenario, you may use
     ...
     * {@link Response}, check timeout using {@link Response#isTimeout()}</li>
     * <li>Call {@link Workflow#savepoint()} to save the workflow state, to avoid duplicated execution during process
     * crash/restart, but note that any wait will do this</li>
     * <li>Make sure all subflows have their independent workflow data, to avoid concurrency issues</li>
     * <li>Remember to reload data (i.e. Order) if the data might be changed after a "wait/sleep"</li>
     * <li>Must handle exceptions from external calls, rollback operation by invoking reverse methods when necessary
     * (i.e. externalService.createSomething() reversed by externalService.deleteSomething())</li>
     * <li>if need to use for-loops in workflow, always use <b>for(int i;i&lt;c.size();i++)</b> instead of
     * <strike>for(element:c)</strike> notation</li>
     * 
     * @throws Interrupt
     */
